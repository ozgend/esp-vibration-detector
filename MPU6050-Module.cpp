...
void MPU6050Module ::initialize()
{
  ...
  /*
  writeI2C(_deviceAddress, MPU6050_REGISTER_GYRO_X_OFFSET, 220);
  writeI2C(_deviceAddress, MPU6050_REGISTER_GYRO_Y_OFFSET, 76);
  writeI2C(_deviceAddress, MPU6050_REGISTER_GYRO_Z_OFFSET, -85);
  */
}
...
void MPU6050Module ::readSensorValues(MPU6050Data *data, bool checkThreshold)
{
  ...
  // |w(t)| = √(wx2+wy2+wz2)
  data->Vector = (float)sqrt(pow((float)data->GyroX/GW_SCALING_FACTOR, 2) + pow((float)data->GyroY/GW_SCALING_FACTOR, 2) + pow((float)data->GyroZ/GW_SCALING_FACTOR, 2));
...
bool MPU6050Module::isThresholdExceeded(MPU6050Data *data)
{
  ...
  if(data->GyroX == -1 ||  data->GyroY == -1 || data->GyroZ == -1 || data->GyroX == 0 || data->GyroY == 0 || data->GyroZ == 0)
  {
    return false;
  }
